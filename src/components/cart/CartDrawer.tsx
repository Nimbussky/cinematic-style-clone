      ...
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-6 py-6 border-b border-[#E8E4DC]">
            ...
            <button onClick={toggleCart} className="text-[#5A5A6E] hover:text-[#1A1A2E]">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
            {items.length === 0 ? (
              <div className="text-center py-12">
                ...
                <Link href="/products" onClick={toggleCart} className="inline-block px-6 py-3 bg-[#1A1A2E] text-[#FAFAF6] rounded-xl text-sm">
                  Browse Products
                </Link>
              </div>
            ) : (
              items.map((item) => (
                <div key={item.product.id} className="flex gap-4 p-4 bg-white rounded-2xl">
                  <div className="w-20 h-20 flex-shrink-0">
                    <ProductImage src={item.product.images[0]} alt={item.product.name} className="w-full h-full rounded-xl overflow-hidden" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-medium text-[#1A1A2E] truncate">{item.product.name}</h3>
                    <p className="text-sm text-[#C9A84C] mt-1">{formatPrice(item.product.price)}</p>
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex items-center border border-[#E8E4DC] rounded-lg">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center text-[#5A5A6E]"
                        >
                          -
                        </button>
                      ...
                      <button onClick={() => removeItem(item.product.id)} className="text-[#9A9AAE] hover:text-[#C44A4A]">
          ...
          {items.length > 0 && (
            <div className="px-6 py-6 border-t border-[#E8E4DC] space-y-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-[#1A1A2E] font-medium">{formatPrice(subtotal)}</span>
              </div>
              {subtotal < SITE_CONFIG.shipping.freeMinimum && (
                <p className="text-xs text-[#9A9AAE]">
                  Add {formatPrice(SITE_CONFIG.shipping.freeMinimum - subtotal)} more for free shipping
                </p>
              )}
              <Link
                href="/checkout"
